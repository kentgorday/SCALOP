...
def anarci_fun(list_of_inputs, numbering_scheme):  # for parallel pool
    ...
    for chain in list_of_inputs:
        anarcioutput = run_anarci(
            list_of_inputs[chain], scheme=numbering_scheme, assign_germline=False, ncpu=8
        )
...
def get_PSSM(storepos):
    ...
    pssm = {pos: np.log2(np.array(storepos[pos]) / 0.05) for pos in sorted(storepos)}
    return pssm
...
def write_txt(assignresults, outputdir, scheme, definition, graftedstructs=None, opf=""):
    ...
    if graftedstructs:
        lineout[-1] += "\tGrafted_Loop"
    if opf == "":
        opf = os.path.join(outputdir, f"{time.time()}.txt")
    for i in range(len(assignresults)):
        for cdr in sorted(assignresults[i]["outputs"]):
            lineout.append(
                "\t".join([assignresults[i]["seqname"]] + assignresults[i]["outputs"][cdr])
            )
            if graftedstructs and cdr in graftedstructs:
                lineout[-1] += "\t" + graftedstructs[cdr]
