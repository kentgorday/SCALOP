...
from scalop.prosci.loops.fread_meld import add_oxygens, meld
from scalop.prosci.util.pdb import Pdb, residueCode
from scalop.prosci.util.residue import Residue, ResidueList
...
def miniFREAD(inputseq, esst, cmlist, freaddbcdr, blacklist=[]):
    ...
    for cm, cmseq, cmdih, a1, a2, a3, a4 in freaddbcdr:
        if cm not in cmlist or cm in blacklist or len(cmseq) - 4 != len(inputseq):
            continue
        essstore.append(
            (
                score_sequence(cmseq[2:-2], cmdih[2:-2]),
                cm,
                l2obj([np.asarray(a1), np.asarray(a2), np.asarray(a3), np.asarray(a4)]),
            )
        )
    essstore.sort(reverse=True)
    return [
        cminfo for cminfo in essstore if cminfo[0] == essstore[0][0]
    ]  # cluster members with the maximum ess
...
def export_structure(args, assignmentresults, output=""):
    ...
    if (args.hc != "" and args.hc not in p_istructure.get_chain_codes()) or (
        args.lc != "" and args.lc not in p_istructure.get_chain_codes()
    ):
    ...
    if args.dbv == "latest":
        for n in os.listdir(os.path.join(scalop_path, "database")):
            if (
                re.match(rf"{args.scheme}_{args.definition}_v(\d+)-(\d+).pickle", n)
                == None
            ):
                continue
            nt = datetime.datetime.strptime(
                "".join(
                    re.match(
                        rf"{args.scheme}_{args.definition}_v(\d+)-(\d+).pickle", n
                    ).groups()
                ),
                "%Y%m",
            )
        ...
        fname = f"{args.scheme}_{args.definition}_v{args.dbv}.pickle"

    with open(os.path.join(scalop_path, "database", fname), "rb") as f:
        _, _, cm = pickle.load(f, encoding="latin1")
    with open(
        os.path.join(
            scalop_path, "database", f"{args.scheme}_{args.definition}.freaddb"
        )
    ) as f:
        freaddb = json.load(f)
    ...
    for scalopres in assignmentresults:
        ...
        if not iscompletestruc:
            ...
            if len(localpdb.get_seq()) + sum(cdrlens) != len(scalopres["input"][1]):
                print(
                    "The input structure ({}) and CDR sequences ({}) do not match with input sequence ({})".format(
                        len(localpdb.get_seq()), sum(cdrlens), len(scalopres["input"][1])
                    )
                )
                continue
            cdri = 0
        for cdr, loop, cl, _ in sorted(scalopres["outputs"].values()):
            ...
            if iscompletestruc:
                lsi, lei = (
                    scalopres["input"][1].index(loop),
                    scalopres["input"][1].index(loop) + len(loop),
                )
            else:
            ...
            if hasattr(args, "blacklist"):
                maxessanchor = miniFREAD(
                    loop, ess, cm[cdr][cl]["Cluster members"], freaddb[cdr], args.blacklist
                )
            else:
            ...
            p_loop = Pdb(
                str(
                    os.path.join(
                        args.loopdb,
                        f"{args.scheme}_{args.definition}",
                        cdr,
                        loopname + ".pdb",
                    )
                )
            ...
            add_oxygens(
                rloop[-1:] + i_Cn + [ri[lei + 3]], force=True
            )  # Add oxygen to C-anchor, and add the i+1th residue to the C-terminus where i is anchor length
            rename_chain(rloop, c)

            for loopi in range(len(rloop)):
                rloop[loopi].set_type(residueCode(loop[loopi]))
            if c not in loopstograft:
                loopstograft.update({c: []})
            loopstograft[c].append(
                (
                    lsi,
                    cut_side_chains(ResidueList(i_Nn).deep_copy()),
                    cut_side_chains(ResidueList(rloop).deep_copy()),
                    cut_side_chains(ResidueList(i_Cn).deep_copy()),
                    lei,
                    ResidueList(ri).deep_copy(),
                )
            )
        if output != "":
            model = ResidueList([])
            for c in loopstograft:
                ...
                for lsi, i_Nn, rloop, i_Cn, lei, ri in sorted(loopstograft[c]):
                    ...
                    end_of_segment = (
                        len(ri) if cloopi == numcdr else sorted(loopstograft[c])[cloopi][0] - 2
                    )
                    ...
                    if cloopi != numcdr:
                        starti = sorted(loopstograft[c])[cloopi][0] - 2
