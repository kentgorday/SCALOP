...
def loop_parse(s, lanchor, lcdr, ucdr, uanchor, cdr, Abchain, bfactor):
    ...
    for r in s.get_residues():
        if (
            hasattr(r, "chain_type")
            and r.chain_type == cdr[-2]
            and r.id[1] >= lanchor
            and r.id[1] <= uanchor
            and r.get_full_id()[3].lower() == Abchain.lower()
            and r.get_resname() in res_dict
        ):
            for atom in r:
                if (
                    r.id[1] >= lcdr
                    and r.id[1] < ucdr
                    and atom.name in ["CA", "C", "N", "O"]
                    and atom.bfactor >= bfactor
                ):  # only those in the CDR loop backbone
                    return []
...
def runbuilddb(args):
    ...
    numnewchain = 0
    print(
        "Screening through SAbDab to find antibody structures with resolution <= %1.1f angstrom"
        % struc_cutoff
    )
    # import Antibodies database (all)
    if os.path.exists(os.path.join(dbdir, "Abs.obj")):
        with open(os.path.join(dbdir, "Abs.obj")) as f:
            Abs = pickle.load(f)
    else:
        Abs = {}
    for entry in sorted(db):
        if entry in bannedlist:
            continue
        p = db.fetch(entry)

        # ignore if resolution > struc_cutoff
        try:
            if p.get_resolution().isalpha() or float(p.get_resolution()) > struc_cutoff:
                continue
        except ValueError:
            continue
    ...
    with open(os.path.join(dbdir, "Abs.obj"), "wb") as f:
        pickle.dump(Abs, f)
    print(f"Found {len(Abs)} high-quality antibodies")
    ...
    print(
        "Creating %d (non-daemon) workers and jobs in main process." % (min(maxproc, len(joblist)))
    )
        ...
        for cdr in cdrs:
            ...
            try:
                ...
                if (
                    sum(
                        [
                            [a.accept(num, chain) for num in missingr[chain]].count(1)
                            for chain in missingr
                        ]
                    )
                    != 0
                ):
                    continue
                cdrseq, anchor = getnumberedCDRloop_builddb(
                    list(allVanarcioutput[cdr[-2]][Abid[0]].items()),
                    "CDR" + cdr,
                    args.scheme,
                    args.definition,
                )
                ...
                residuelist = loop_parse(
                    structure, lanchor, startpos, endpos, uanchor, cdr, Abid[0][-1], args.bfactor
                )  # Higher quality loops only.
                ...
                pdboutf.write("TER\nEND\n")
                pdboutf.close()
                if os.path.getsize(outfdir) == 0:
                    os.system(f"rm -rf {outfdir}")
...
def clustering(args):
    ...
    if "ReadDir_DTW.o" not in os.listdir("builddb_code"):
        if args.arma_inc != "":
            cmd.append(
                f"g++ --std=c++11 builddb_code/ReadDir_DTW.cpp -o builddb_code/ReadDir_DTW.o -I{args.arma_inc} -lboost_system -lboost_filesystem -DARMA_USE_LAPACK -DARMA_USE_BLAS -DARMA_DONT_USE_WRAPPER -llapack -lblas"
            )
        else:
            cmd.append(
                "g++ --std=c++11 builddb_code/ReadDir_DTW.cpp -o builddb_code/ReadDir_DTW.o -lboost_system -lboost_filesystem -DARMA_USE_LAPACK -DARMA_USE_BLAS -DARMA_DONT_USE_WRAPPER -llapack -lblas"
            )

    if os.path.exists("%s/results/" % (args.dbdir)) == False:
        os.mkdir("%s/results/" % (args.dbdir))
    ...
    for cdr in cdrs:
        if cdr != cdrs[-1]:
            cmd.append(
                f"(./builddb_code/ReadDir_DTW.o {args.dbdir}/{cdr} {args.dbdir}/results/{cdr}filelist {args.dbdir}/results/{cdr}distmatrix ; python builddb_code/DTW_vis_fixedcut-offs.py --directory={args.dbdir}/{cdr} --filelist={args.dbdir}/results/{cdr}filelist --distfile={args.dbdir}/results/{cdr}distmatrix )&"
            )
        else:  # Last one
            cmd.append(
                f"(./builddb_code/ReadDir_DTW.o {args.dbdir}/{cdr} {args.dbdir}/results/{cdr}filelist {args.dbdir}/results/{cdr}distmatrix ; python builddb_code/DTW_vis_fixedcut-offs.py --directory={args.dbdir}/{cdr} --filelist={args.dbdir}/results/{cdr}filelist --distfile={args.dbdir}/results/{cdr}distmatrix );"
    ...
    for cdr in cdrs:
        cmd.append(
            "mv "
            + os.path.join(args.dbdir, "results/Cluster_data%s_%sA.txt" % (cdr, cutoffs[cdr]))
            + " "
            + os.path.join(
                args.dbdir,
                "results/Cluster_data%s_%sA_%s_%s.txt"
                % (cdr, cutoffs[cdr], lastm.strftime("%Y"), lastm.strftime("%m")),
            )
        )
    # bash file
    bashfn = f"compile_and_run_{time.time()}.sh"
...
def extractcluster(args):
    ...
    for cdr in cdrs:
        ...
        for clno in allclusters:
            ...
            allloops = [
                Ab.split(".")[0]
                for Ab in list(ClusterInfo[ClusterInfo["Cluster"] == clno]["Loop_name"])
            ]
            lengthstore = []
            if tempclustername == "UC":
                ...
                clustercenter = [
                    ...
                    for Ab in list(
                        ClusterInfo[
                            (ClusterInfo["Cluster"] == clno)
                            & (ClusterInfo["Cluster_center"] == "Yes")
                        ]["Loop_name"]
                    )
                ]
                ...
                for Abid in allloops:
                    ...
                    if Abid not in clusterloopstore[cdr][tempclustername]:
                        [loop, _] = getnumberedCDRloop(
                            list(allVanarcioutput[chain][Abid].items()),
                            cdr,
                            args.scheme,
                            args.definition,
                        )
                            ...
                            sum(
                                [
                                    identity(
                                        ...
                                        {
                                            chain: dict(
                                                clusterloopstore[cdr][tempclustername][Abother]
                                            )
                                        },
                                        scheme=args.scheme,
                                    ...
                                    == 1.00
                                    for Abother in clusterloopstore[cdr][tempclustername]
                                ]
                            )
                            != 0
                        ):
                            continue  # skip if not unique
                        clusterloopstore[cdr][tempclustername].update(
                            {Abid: loop}
                        )  # only store unique loops
                        if len(loop) not in lengthstore:
                            lengthstore.append(len(loop))
            ...
            if tempclustername not in clusterout[cdr]:
                clusterout[cdr].update({tempclustername: {"Cluster members": [], "Lengths": []}})
        ...
        for cl in clusterout[cdr]:
            ...
            if lengths not in clusters:
                clusters.update({lengths: {}})
            clusters[lengths].update({cl: len(clusterloopstore[cdr][cl])})
        for lengths in clusters:
            asciiindex = 0
            for cl, size in sorted(
                list(clusters[lengths].items()), key=operator.itemgetter(1), reverse=True
            ):
        ...
        for clustername in list(clusterabstore[cdr].keys()):
            ...
            for Abid in clusterloopstore[cdr][clustername]:
                for pos, residue in clusterloopstore[cdr][clustername][Abid]:
                    ...
                    if pos not in storepos:
                        storepos[pos] = np.zeros(20)
        ...
        if cdr == "L2":
            ...
            for cl in list(clusterout[cdr].keys()):
                if cl != largestcl and cl != "UC":
                    clusterout[cdr]["UC"]["Cluster members"] += clusterout[cdr][cl][
                        "Cluster members"
                    ]
    ...
    with open(
        os.path.normpath(os.path.join(folderdir + "/ClusterMember.json")), "wb"
    ) as f:  # a little weird hack
        json.dump(clusterout, f)

    newdbname = os.path.join(
        scalop_path,
        "database",
        "{0}_{1}_v{2}-{3}.pickle".format(
            args.scheme, args.definition, lastm.strftime("%Y"), lastm.strftime("%m")
        ),
    )
...
def update(args):
    args.dbdir = os.path.join(args.dbdir, f"{args.scheme}_{args.definition}")
    try:
        ...
        if input("Have you installed SAbDAb?"):
            ...
            args.sabdabpydir = input(
                "Please enter the location of ABDB python module (e.g. ~/bin/Python/ABDB):"
            )
            sys.path.append(args.sabdabdir)
            sys.path.append(args.sabdabpydir)
        else:
            ...
            sys.exit(1)
    if args.sabdabu == "yes":
        os.system(os.path.join(args.sabdabdir, "SAbDab") + " -u")
